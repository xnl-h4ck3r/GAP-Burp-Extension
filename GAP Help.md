...
<li><b>Show origin</b> - If this feature is ticked, the potential parameter will be followed by the HTTP request endpoint (in square brackets) that the parameter was found in. A parameter could have been found in more than one request, so this view can show duplicate links, one per origin endpoint. If the parameter was included because the <b>Include common parameters?</b> option was checked, then it will be followed by <code>[GAP]</code> instead of an origin endpoint.</li>
